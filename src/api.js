...
export default class Api extends React.Component {
    ...
    state = {
        ...
        useAddr: "0x2b1803f88b660294de17b2c3de919339db938969",
        param: this.urlAddr
      };
    ...
    async componentDidMount() {
        ...
        const myContract = new web3.eth.Contract(ABI_JSON, '0x2152aa45e02c1022cAB4ad964663fB3D69C2023e', {
            from: this.state.useAddr // default from address
        });// create web3 contract object using ABI pulled from bscscan
        const polyContract = new web3p.eth.Contract(ABI_JSON,'0x8c351D4AB9922dF476384BeBdF45eCFC19782773', {
            from: this.state.useAddr
        });//firefund polygon contract
        const polyPrice = new web3p.eth.Contract(ABI_PRICE,'0xAB594600376Ec9fD91F8e885dADF0CE036862dE0', {
            from: this.state.useAddr
        });//chainlink Matic price feed
        const ftmContract = new web3f.eth.Contract(ABI_JSON,'0xFB9D801bc58488a9Cb7FfBEdBB82350705c7e01d', {
            from: this.state.useAddr
        })//firefund fantom contract
        const ftmPrice = new web3f.eth.Contract(ABI_PRICE,'0xf4766552D15AE4d256Ad41B6cf2933482B0680dc', {
            from: this.state.useAddr
        });//chainlink Fantom price feed
        const arbContract = new web3a.eth.Contract(ABI_JSON,'0x515579FB6F65EE38eb0E401725079a955a3281B8', {
            from: this.state.useAddr
        })//firefund arbitrum price feed
        const arbPrice = new web3a.eth.Contract(ABI_PRICE,'0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612', {
            from: this.state.useAddr
        ...
        this.setState({ tRewPUSD: Math.round(pTotRew*pPrice*100)/100, loading: false});
        this.setState({ tStakeP: valP, loading: false});
        this.setState({ tStakePUSD: Math.round(valP*pPrice*100)/100, loading: false});
        this.setState({ tRewF: fTotRew, loading: false});
        this.setState({ tRewFUSD: Math.round(fTotRew*fPrice*100)/100, loading: false});
        this.setState({ tStakeF: valF, loading: false});
        this.setState({ tStakeFUSD: Math.round(valF*fPrice*100)/100, loading: false});
        this.setState({ tRewA: aTotRew, loading: false});
        this.setState({ tRewAUSD: Math.round(aTotRew*aPrice*100)/100, loading: false});
        this.setState({ tStakeA: valA, loading: false});
        this.setState({ tStakeAUSD: Math.round(valA*aPrice*100)/100, loading: false});
        this.setState({ tRefB: Math.round(refB/10000000000000000)/100, loading: false});
        this.setState({ tRefP: Math.round(refP/10000000000000000)/100, loading: false});
        this.setState({ tRefPUSD: Math.round(refP*pPrice/10000000000000000)/100, loading: false});
        this.setState({ tRefF: Math.round(refF/10000000000000000)/100, loading: false});
    ...
    render () {
        ...
        return (
            <div>
                ...
                <div>
                    <input type="text" defaultValue="0x2b1803f88b660294de17b2c3de919339db938969" onChange={this.handleChange} style={{ width: "350px" }}/>
                    <br></br>
                    <button type="submit" onClick={this.handleClick}>Check Address</button>
                </div>
                <h4>{this.state.useAddr}</h4>
                <style>{"table th{border:1px solid black;} table td{border:1px solid black;}"}</style>
                <table>
                    <tr>
                        <th>Chain</th>
                    ...
                    <tr>
                        <th>TOTAL</th>
                        <th>{Math.round((
                        this.state.tStake+this.state.tStakePUSD+this.state.tStakeFUSD+this.state.tStakeAUSD
                        )*100)/100} $USD</th>
                        <th>{Math.round((
                        this.state.tRew+this.state.tRewPUSD+this.state.tRewFUSD+this.state.tRewAUSD
                        )*100)/100} $USD</th>
                        <th>{Math.round((
                        this.state.tRefB+this.state.tRefPUSD+this.state.tRefFUSD+this.state.tRefAUSD
                        )*100)/100} $USD</th>
                    </tr>
                </table>
                <br></br>
